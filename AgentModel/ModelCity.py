...
class CityModel(mesa.Model):
        ...
        for buildingPos in self.walkableBuildings:
            ...
            for c in cell:
                if isinstance(c, Building):
                    c.walkable = True
    def addBusStop(self):
        """Añadir paradas de autobús a la cuadrícula."""
        for busStop, (x,y) in self.busStops.items():
            agent = BusStop(self.next_id(), self, busStop)
    ...
    def getAllData(self):
        ...
        for people in self.pedestrians:
            result["pedestrians"].append({"id":people.unique_id,"x":people.pos[0],"z":people.pos[1],"crossing":people.crossing,"inBus":people.inBus})
